            ...
            for room_id in os.listdir(perspective_path):
                ...
                for position_id in os.listdir(room_path):
                    ...
                    if os.path.exists(layout_file):
                        ...
                        depth_filename = os.path.join(position_path, "plane_depth.png")  # Changed name to plane_depth.png
...
def main():
    parser = argparse.ArgumentParser(description='Process Structured3D dataset to generate depth maps')
    parser.add_argument('--path', type=str, required=True, 
                      help='Path to the Structured3D dataset root directory')
    args = parser.parse_args()
    
    if not os.path.exists(args.path):
        raise ValueError(f"Dataset path does not exist: {args.path}")
