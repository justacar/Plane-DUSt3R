...
def forward_mast3r(pairs, model, cache_path, desc_conf='desc_conf',
                   device='cuda', subsample=8,verbose=False, **matching_kw):
    res_paths = {}
    pair_iterator = tqdm(pairs) if verbose else pairs
    for img1, img2 in pair_iterator:
...
def prepare_canonical_data(imgs, tmp_pairs, subsample, order_imgs=False, min_conf_thr=0,
    ...
    for img in tqdm(imgs,disable=not verbose):
        if cache_path:
...
@torch.no_grad()
def spectral_projection_of_depthmaps(imgs, intrinsics, depthmaps, subsample, cache_path=None,verbose=False, **kw):
    # recover 3d points
    core_depth = []
    lora_proj = []
    if verbose:
        pair_iterator = tqdm(imgs)
    else:
        pair_iterator = imgs
    for i, img in enumerate(pair_iterator):
